...
test('R14 cutover materializer is confirmation-gated, scoped and monotone', () => {
  ...
  assert.match(cutoverTool, /const CACHE_VERSION = 'v24'/);
  assert.match(cutoverTool, /activity-product-controller\.js\?v=24/);
  ...
  assert.match(cutoverTool, /\$chartAsset = "  toUrl\('app\/modules\/doctor-stack\/charts\/index\.js\?v=24'\),"/);
...
test('R14 productload is one ordered V2 writer graph with exact v24 cache identities', () => {
  ...
  for (const relativePath of captureOrder) {
    ...
    assert.equal(count(index, `src="${relativePath}?v=24"`), 1, relativePath);
    assert.equal(count(worker, `toUrl('${relativePath}?v=24')`), 1, relativePath);
    previousIndex = indexPosition;
    previousWorker = workerPosition;
  }
  assert.ok(previousIndex < index.indexOf('src="app/supabase/index.js?v=24"'));
  for (const relativePath of stylePaths) {
    const importPath = relativePath.replace(/^app\//, './');
    assert.equal(count(css, `@import url("${importPath}?v=24")`), 1, relativePath);
    assert.equal(count(worker, `toUrl('${relativePath}?v=24')`), 1, relativePath);
  }
  assert.match(index, /href="app\/app\.css\?v=24"/);
  assert.match(index, /src="assets\/js\/main\.js\?v=24"/);
  assert.match(index, /src="app\/supabase\/index\.js\?v=24"/);
  assert.match(index, /src="app\/modules\/doctor-stack\/charts\/index\.js\?v=24"/);
  assert.match(worker, /toUrl\('app\/modules\/doctor-stack\/charts\/index\.js\?v=24'\)/);
  assert.match(worker, /test\(new URL\(request\.url\)\.pathname\)/);
  assert.match(worker, /const CACHE_VERSION = 'v24'/);
...
test('R14 rollback material restores only explicit baseline product paths and creates v25', () => {
  ...
  assert.match(rollback, /Set-ReleaseModuleVersion \$relativePath '25'/);
  assert.match(rollback, /const CACHE_VERSION = 'v25'/);
  assert.match(rollback, /app\/modules\/vitals-stack\/activity\/index\.js\?v=25/);
  assert.match(rollback, /app\/modules\/doctor-stack\/charts\/index\.js\?v=25/);
  assert.equal(count(gitShow(baselineCommit, 'service-worker.js'), "toUrl('app/modules/doctor-stack/charts/index.js')"), 0);
  assert.match(rollback, /\$chartAsset = "  toUrl\('app\/modules\/doctor-stack\/charts\/index\.js\?v=25'\),"/);
  ...
  let rollbackWorker = gitShow(baselineCommit, 'service-worker.js')
    .replace("const CACHE_VERSION = 'v13';", "const CACHE_VERSION = 'v25';")
    .replace(
  ...
  rollbackWorker = rollbackWorker
    .replace("toUrl('app/app.css')", "toUrl('app/app.css?v=25')")
    .replace("toUrl('app/supabase/index.js')", "toUrl('app/supabase/index.js?v=25')")
    .replace("toUrl('assets/js/main.js')", "toUrl('assets/js/main.js?v=25')")
    .replace("toUrl('app/modules/vitals-stack/activity/index.js')", "toUrl('app/modules/vitals-stack/activity/index.js?v=25')");
  assert.match(rollbackWorker, /const CACHE_VERSION = 'v25'/);
  assert.equal(count(rollbackWorker, "toUrl('app/modules/vitals-stack/activity/index.js?v=25')"), 1);
