...
const roots = Object.freeze({
  ...
  v24: process.env.R14_RELEASE_V24_ROOT,
  v25: process.env.R14_RELEASE_V25_ROOT
});
...
(async () => {
  ...
  try {
    ...
    await startControlled(stalePage, base, roots.v23);
    await installWaiting(stalePage, roots.v24);
    await stalePage.goto(`${base}?transition=v23-v24`, { waitUntil: 'networkidle' });
    await inspectRelease(stalePage, '24', true);
    await staleContext.close();

    const rollbackContext = await browser.newContext({ serviceWorkers: 'allow' });
    const rollbackPage = await rollbackContext.newPage();
    await startControlled(rollbackPage, base, roots.v24);
    await installWaiting(rollbackPage, roots.v25);
    await rollbackPage.goto(`${base}?transition=v24-v25`, { waitUntil: 'networkidle' });
    await inspectRelease(rollbackPage, '25', false);
    await rollbackContext.close();

    const offlineContext = await browser.newContext({ serviceWorkers: 'allow' });
    const offlinePage = await offlineContext.newPage();
    await startControlled(offlinePage, base, roots.v24);
    ...
    const offlineUrls = await offlinePage.evaluate(() => performance.getEntriesByType('resource')
      .map((entry) => entry.name)
      .filter((url) => url.includes('/app/supabase/') || url.includes('/assets/js/boot-auth.js')));
    assert.ok(offlineUrls.every((url) => new URL(url).search === '?v=24'));
    assert.equal(new Set(offlineUrls.map((url) => new URL(url).pathname)).size,
      supabaseGraph.length);
    await offlineContext.close();

    process.stdout.write(
      'R14_ACTUAL_RELEASE_TRANSITION_PASS stale_v23_v24=1 rollback_v24_v25=1 fresh_v24_offline=1\n'
    );
  } finally {
    await browser.close();
