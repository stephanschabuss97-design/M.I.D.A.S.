'use strict';
/**
 * MODULE: touchlog/index.js
 * Description: Touchlog maintenance surface controller.
 * Notes:
 *  - Uses localStorage flags where possible.
 *  - Push control uses AppModules.push.
 */

(function initTouchlogModule(global) {
  ...
  const updatePushToggle = async (el, statusEl = null, reason = 'touchlog') => {
    if (!el) return;
    const detailsEl = doc.querySelector(selectors.pushDetails);
    try {
      const push = getOperationalPushApi();
      if (push) {
        const routingFromRefresh = await push.refreshPushStatus({ reason });
  ...
  const bind = () => {
    ...
    state.activeModesEl = activeModesEl;
    state.controls = {
      sound: soundEl,
      haptic: hapticEl,
      nocache: nocacheEl,
  ...
  const refreshPushStatus = async (reason = 'touchlog') => {
    const pushEl = doc.querySelector(selectors.push);
    const pushStatusEl = doc.querySelector(selectors.pushStatus);
    await updatePushToggle(pushEl, pushStatusEl, reason);
  };

  const updateActiveModes = () => {
    renderActiveModes(state.activeModesEl, [
      { label: 'Sound', active: !!state.controls.sound?.checked },
      { label: 'Haptik', active: !!state.controls.haptic?.checked },
      { label: 'No Cache', active: !!state.controls.nocache?.checked },
      { label: 'Assistant', active: !!state.controls.assistant?.checked },
    ]);
  };

  const add = (message) => {
    const diag = getDiag();
    if (typeof diag?.add === 'function') {
      diag.add(message);
    }
  };

  global.AppModules.touchlog = Object.assign(global.AppModules.touchlog || {}, {
    init,
    refreshPushStatus,
    updateActiveModes,
    add,
