-- MIDAS Activity V2 R8: guarded exact R2 commit-RPC rollback.
--
...
-- This rollback is a separate owner-gated action; it accepts only exact R8.
...
begin
  ...
  if v_source_sha256 not in ('7cdabca31dd7b4f3a8a78f5dc4d79c2116c7f77a2a0f5b834439093c0215177e') then
    raise exception 'Activity V2 SQL 22 rollback source preimage drift: %',
      v_source_sha256;
  end if;
...
begin
  select pg_catalog.encode(
    ...
    into strict v_source_sha256;
  if v_source_sha256 <> '2241cea9a5453a38d074abc88aebe8edb6f7e5c0226d063423daef0b1411418e' then
    raise exception 'Activity V2 SQL 22 rollback source postcondition failed: %',
      v_source_sha256;
  end if;
  if not exists (
    select 1
    ...
    raise exception 'Activity V2 SQL 22 rollback hardening postcondition failed';
  end if;
  select pg_catalog.jsonb_agg(
           pg_catalog.jsonb_build_array(
             case when acl.grantee = 0 then 'PUBLIC' else grantee.rolname end,
   ...
   where p.oid = v_commit_oid;
  if v_acl <> '[["authenticated","postgres","EXECUTE",false],["postgres","postgres","EXECUTE",false]]'::jsonb then
    raise exception 'Activity V2 SQL 22 rollback ACL postcondition failed';
  end if;
end;
$post$;
...
begin
  if not exists (
    ...
    raise exception 'Activity V2 SQL 22 rollback changed protected state';
  end if;
end;
$unchanged$;
