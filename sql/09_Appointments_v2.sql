...
drop view if exists public.v_appointments_v2_upcoming;
drop table if exists public.appointments_v2 cascade;
drop function if exists public.set_appointments_v2_updated_at();
...
create index idx_appt_v2_user_start
  on public.appointments_v2(user_id, start_at desc);

create index idx_appt_v2_status
...
create policy appointments_v2_select
  on public.appointments_v2 for select
  using ((select auth.uid()) = user_id);

create policy appointments_v2_insert
  on public.appointments_v2 for insert
  with check ((select auth.uid()) = user_id);

create policy appointments_v2_update
  on public.appointments_v2 for update
  using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);

create policy appointments_v2_delete
  on public.appointments_v2 for delete
  using ((select auth.uid()) = user_id);
...
create view public.v_appointments_v2_upcoming as
select
  a.id,
  a.user_id,
  a.title,
...
comment on view public.v_appointments_v2_upcoming
  is 'Hilfsview fuer Apps: liefert kommende Termine ab gestern +1 Tag.';
