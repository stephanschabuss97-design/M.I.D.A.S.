-- MIDAS Activity V2 R13: activate the shared read-consumer runtime.
--
-- This DML-free transition preserves the public SQL 25 signature, moves the
-- one V1/V2 union into a private owner-parametrized core and adds one
-- service-role-only wrapper for the two guarded scheduler consumers.
...
begin
  if pg_catalog.current_setting('server_version_num')::integer
    ...
    raise exception 'Activity consumer SQL 26 requires PostgreSQL 17';
  end if;
  if session_user <> 'postgres' or current_user <> 'postgres' then
    raise exception 'Activity consumer SQL 26 requires postgres session and current user';
  end if;
  if v_owner is null or v_authenticated is null or v_service_role is null
     or pg_catalog.to_regrole('anon') is null then
    raise exception 'Activity consumer SQL 26 API role preimage drift detected';
  end if;
  if pg_catalog.to_regnamespace('midas_private') is null or v_helper_oid is null
     or not exists (
       select 1
         from pg_catalog.pg_namespace n
        where n.oid = 'midas_private'::pg_catalog.regnamespace
          and n.nspowner = v_owner
     ) then
    raise exception 'Activity consumer SQL 26 private-schema preimage drift detected';
  end if;
  if not pg_catalog.has_schema_privilege(
       'authenticated', 'midas_private', 'USAGE'
     )
     or pg_catalog.has_schema_privilege('anon', 'midas_private', 'USAGE')
     or pg_catalog.has_schema_privilege('authenticated', 'midas_private', 'CREATE')
     or pg_catalog.has_schema_privilege('service_role', 'midas_private', 'CREATE')
    ...
    raise exception 'Activity consumer SQL 26 R9 ACL preimage drift detected';
  end if;
  if (select pg_catalog.count(*)
     ...
     or v_user_oid is null then
    raise exception 'Activity consumer SQL 26 user-wrapper overload drift detected';
  end if;
  if (select pg_catalog.count(*)
        from pg_catalog.pg_proc p
        join pg_catalog.pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'public'
         and p.proname = 'activity_consumer_snapshot_for_owner') <>
       (case when v_service_oid is null then 0 else 1 end)
     or (select pg_catalog.count(*)
           from pg_catalog.pg_proc p
           join pg_catalog.pg_namespace n on n.oid = p.pronamespace
          where n.nspname = 'midas_private'
            and p.proname = 'activity_consumer_snapshot_core') <>
       (case when v_core_oid is null then 0 else 1 end) then
    raise exception 'Activity consumer SQL 26 service/core overload drift detected';
  end if;

  if v_core_oid is null and v_service_oid is null then
    if pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(
      ...
      raise exception 'Activity consumer SQL 26 SQL 25 preimage drift detected';
    end if;
  elsif v_core_oid is not null and v_service_oid is not null then
    if (select pg_catalog.count(*)
          from pg_catalog.pg_proc p
          join pg_catalog.pg_namespace n on n.oid = p.pronamespace
         where (n.nspname = 'public'
            ...
            or (n.nspname = 'midas_private'
                and p.proname = 'activity_consumer_snapshot_core')) <> 2 then
      raise exception 'Activity consumer SQL 26 rerun overload drift detected';
    end if;
    if pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(
         pg_catalog.pg_get_functiondef(v_user_oid), 'UTF8')), 'hex') <>
      ...
      raise exception 'Activity consumer SQL 26 rerun source/hardening/ACL drift detected';
    end if;
  else
    raise exception 'Activity consumer SQL 26 partial preimage drift detected';
  end if;
end;
$guard$;

create temporary table midas_activity_consumer_sql26_preimage
on commit drop
as
select
...
create or replace function midas_private.activity_consumer_snapshot_core(
  p_owner uuid,
  p_from date,
  p_to date
)
  ...
  if exists (
    select 1
      ...
      join public.health_activity_session_items i
        on i.session_id = s.id and i.user_id = p_owner
     where s.user_id = p_owner
       and s.day between p_from and p_to
       and s.started_at >= v_from_at
       and s.started_at < v_until_at
  ...
  v2_item_counts as (
    select i.session_id, pg_catalog.count(*)::integer as item_count
      ...
      join public.health_activity_sessions s
        on s.id = i.session_id and s.user_id = p_owner
...
begin
  ...
  return midas_private.activity_consumer_snapshot_core(v_user, p_from, p_to);
end;
$function$;

create or replace function public.activity_consumer_snapshot_for_owner(
  p_owner uuid,
  p_from date,
  p_to date
)
...
begin
  ...
  return midas_private.activity_consumer_snapshot_core(p_owner, p_from, p_to);
end;
$function$;

alter function midas_private.activity_consumer_snapshot_core(uuid, date, date)
  owner to postgres;
...
begin
  if v_user_oid is null or v_service_oid is null or v_core_oid is null
    ...
    raise exception 'Activity consumer SQL 26 function postcondition failed';
  end if;
  if not pg_catalog.has_schema_privilege('authenticated', 'midas_private', 'USAGE')
       ...
       select 1
        ...
        where n.oid = 'midas_private'::pg_catalog.regnamespace
          and (
            ...
            or (acl.grantee <> pg_catalog.to_regrole('postgres')
     ...
     or exists (
       select 1
        ...
        where p.oid in (v_user_oid, v_service_oid, v_core_oid)
          and (
            acl.grantor <> pg_catalog.to_regrole('postgres')
    ...
    raise exception 'Activity consumer SQL 26 ACL postcondition failed';
  end if;
  if pg_catalog.pg_get_functiondef(v_user_oid) ~* 'union[[:space:]]+all'
     or pg_catalog.pg_get_functiondef(v_service_oid) ~* 'union[[:space:]]+all'
     or pg_catalog.pg_get_functiondef(v_core_oid) !~* 'union[[:space:]]+all' then
    raise exception 'Activity consumer SQL 26 canonical-union postcondition failed';
  end if;
  if (select events_sha256 from midas_activity_consumer_sql26_preimage) <>
       (select pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(
          coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(e) order by e.id), '[]'::jsonb)::text,
          'UTF8')), 'hex') from public.health_events e)
     or (select sessions_sha256 from midas_activity_consumer_sql26_preimage) <>
       (select pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(
          coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(s) order by s.id), '[]'::jsonb)::text,
          'UTF8')), 'hex') from public.health_activity_sessions s)
     or (select items_sha256 from midas_activity_consumer_sql26_preimage) <>
       (select pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(
          coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(i) order by i.id), '[]'::jsonb)::text,
          'UTF8')), 'hex') from public.health_activity_session_items i) then
    raise exception 'Activity consumer SQL 26 changed protected Activity data';
  end if;
end;
$post$;
