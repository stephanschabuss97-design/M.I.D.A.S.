...
- Push-Wartung nutzt die interne Profile-Push-API als technische Quelle:
  - `enablePush`
  - `disablePush`
  - `isPushEnabled`
  - `refreshPushStatus`
...
### 4.3 Verarbeitung
- Push-Control delegiert an die interne Profile-Push-API.
...
- Public Touchlog-Entry-Points:
  ...
  - `diag.add(...)`
  - `diag.clear()`
- Push-Wartung nutzt intern `AppModules.profile`.
...
- Bessere visuelle Health-Komprimierung, falls Mobile-Textfit bei echten Langwerten leidet.
...
- Status: aktiv.
- Dependencies (hard): Diagnostics-Core, Touchlog-Markup, Profile-Push-API, Browser Notification/Service Worker.
