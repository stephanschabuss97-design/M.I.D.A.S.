...
3) Profil-UX: Faktor & Zielbereich sichtbar machen
  ...
  3.2 Doctor-Lock Faktor als Source of Truth (Edge berechnet Range):
      ...
      3.2.3 Edge Function: Wenn Doctor-Lock aktiv -> Faktor = `protein_doctor_factor`,
            sonst Faktor = auto (CKD/Activity/Weight).
      3.2.4 Range-Berechnung vereinheitlichen:
      ...
      3.2.5 Persistenz: speichere `protein_factor_current`, `protein_target_min/max` wie bisher,
            aber Quelle (auto vs doctor) im Payload/Log nachvollziehbar.
      3.2.6 Module Overview nachziehen (Doctor-Lock Faktor-Flow).
  3.3 Optionaler manueller Recompute-Button (force=true) (nach 3.2).
  3.4 Module Overview nachziehen (UI/Display) (letzter Punkt in 3).

4) Assistant-Text: Faktor & Range
  4.1 Assistant zeigt "Protein X-Y g (Faktor Z.ZZ g/kg)".
  4.2 Fallback bleibt `protein_target_*` aus Profil; kein neuer Rechenpfad.
  4.3 Module Overview nachziehen (Assistant-Text).

5) Woechentlicher Recompute (GitHub Actions)
  5.1 Scheduler-Plan: Do->Fr Nacht (UTC beachten).
  5.2 Auth-Strategie definieren (Edge nutzt User JWT):
      - Option A: Service Role + eigener Edge-Auth-Header (analog Trendpilot-Cron).
      - Option B: separates Scheduler-Endpoint mit Secret.
  5.3 Cron-Call implementieren (nur wenn Auth-Struktur steht).
  5.4 Logger/diag fuer Cron-Runs.
  5.5 Module Overview nachziehen (Cron-Trigger).
...
- Cron-Auth: eigener Secret-Header wie Trendpilot, oder anderer Weg?
